...
void MainWindow::on_actionNueva_desde_portapapeles_triggered()
{
    ...
    if (!cb->mimeData()->hasImage()) {
        QMessageBox::warning(this, "Nueva desde el portapapeles", "El portapapeles no contiene una imagen.");
        return;
    }

    QImage qimg = cb->image();
    if (qimg.isNull()) {
        QMessageBox::warning(this, "Nueva desde el portapapeles", "No se pudo obtener la imagen desde el portapapeles.");
        return;
    }

    QImage img = qimg.convertToFormat(QImage::Format_RGBA8888);
    QImage copy = img.copy();
    Mat tmp(copy.height(), copy.width(), CV_8UC4, const_cast<uchar*>(copy.bits()), copy.bytesPerLine());
    Mat res;
    cvtColor(tmp, res, COLOR_RGBA2BGR);
...
void MainWindow::on_actionVer_informacion_triggered()
{
    int fa = foto_activa();
    if (fa == -1) {
        QMessageBox::warning(this, "Ver información", "No hay ninguna imagen activa.");
        return;
    }
    ...
    texto += "<b>Memoria ocupada:</b> " + QString::number(bytes) + " bytes<br/>";
...
{
    if (foto_activa() != -1 && comprobar_primera_libre() != -1) {
...
{
    if (foto_activa() != -1 ){
        copiar_roi_al_portapapeles(foto_activa());
    } else {
        QMessageBox::warning(this, "Copiar al portapapeles", "No hay ninguna imagen activa.");
    }
}
...
void MainWindow::on_actionCapturar_de_v_deo_triggered()
{
    if(primera_libre()!=-1)
    {
        ...
        if(!nombre.isEmpty())
        {
            CapturarVideo cv(nombre.toLatin1().data(), this);
            if(cv.isOpened())
            {
                cv.exec();
            }
...
void MainWindow::on_toolButton_10_triggered(QAction *arg1)
{
    herr_actual = HER_TRAZOS;
    ui->toolButton_10->setChecked(true);
}
