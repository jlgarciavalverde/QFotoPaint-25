...
void ver_perspectiva(int norig, int ndest, Point2f porig[], Point2f pdest[], bool guardar)
{
    ...
    // Draw quadrilaterals and markers: origin in red, destination in green.
    for(int i = 0; i < 4; i++) {
        // Edges (black)
        line(imgorig, porig[i], porig[(i+1)%4], CV_RGB(0,0,0), 2);
        line(res, pdest[i], pdest[(i+1)%4], CV_RGB(0,0,0), 2);
    }

    for(int i = 0; i < 4; i++) {
        // Origin markers: filled red circles with white border
        circle(imgorig, porig[i], 8, Scalar(0,0,255), -1);
        circle(imgorig, porig[i], 10, Scalar(255,255,255), 2);

        // Destination markers: filled green circles with white border
        circle(res, pdest[i], 8, Scalar(0,255,0), -1);
        circle(res, pdest[i], 10, Scalar(255,255,255), 2);
    }
...
// Muestra información relevante de la imagen: tamaño, profundidad,
// número de canales, memoria ocupada y color medio (RGB y recuadro).
void ver_informacion(int nfoto)
{
    if (nfoto < 0 || nfoto >= MAX_VENTANAS || !foto[nfoto].usada) {
        QMessageBox::warning(w, "Ver información", "No hay ninguna imagen en la posición indicada.");
        return;
    }

    Mat &m = foto[nfoto].img;
    if (m.empty()) {
        QMessageBox::warning(w, "Ver información", "La imagen está vacía.");
        return;
    }

    int ancho = m.cols;
    int alto = m.rows;
    int canales = m.channels();
    int profundidad = m.depth();

    QString profStr;
    switch (profundidad) {
    case CV_8U: profStr = "8-bit unsigned (CV_8U)"; break;
    case CV_8S: profStr = "8-bit signed (CV_8S)"; break;
    case CV_16U: profStr = "16-bit unsigned (CV_16U)"; break;
    case CV_16S: profStr = "16-bit signed (CV_16S)"; break;
    case CV_32S: profStr = "32-bit signed (CV_32S)"; break;
    case CV_32F: profStr = "32-bit float (CV_32F)"; break;
    case CV_64F: profStr = "64-bit float (CV_64F)"; break;
    default: profStr = "Desconocida"; break;
    }

    size_t bytes = m.total() * m.elemSize();

    // Intentar obtener tamaño en disco si la imagen tiene un nombre de fichero
    QString fileSizeStr = "(no disponible)";
    if (!foto[nfoto].nombref.empty()) {
        QFileInfo fi(QString::fromLatin1(foto[nfoto].nombref.c_str()));
        if (fi.exists() && fi.isFile()) {
            qint64 fbytes = fi.size();
            auto human = [&](qint64 b)->QString {
                if (b < 1024) return QString::number(b) + " bytes";
                double kb = b / 1024.0;
                if (kb < 1024) return QString::number(kb, 'f', 2) + " KB";
                double mb = kb / 1024.0;
                return QString::number(mb, 'f', 2) + " MB";
            };
            fileSizeStr = human(fbytes) + " (" + QString::number(fbytes) + " bytes)";
        }
    }

    Scalar meanv = mean(m);
    int meanB = cvRound(meanv[0]);
    int meanG = cvRound(meanv[1]);
    int meanR = cvRound(meanv[2]);

    QString texto;
    texto += "<b>Tamaño:</b> " + QString::number(ancho) + " x " + QString::number(alto) + "<br/>";
    texto += "<b>Profundidad:</b> " + profStr + "<br/>";
    texto += "<b>Canales:</b> " + QString::number(canales) + "<br/>";
    auto humanMem = [&](size_t b)->QString {
        if (b < 1024) return QString::number(b) + " bytes";
        double kb = b / 1024.0;
        if (kb < 1024) return QString::number(kb, 'f', 2) + " KB";
        double mb = kb / 1024.0;
    ...
    texto += "<b>Tamaño en memoria:</b> " + humanMem(bytes) + " (" + QString::number(bytes) + " bytes)<br/>";
    texto += "<b>Tamaño en disco:</b> " + fileSizeStr + "<br/>";
    texto += "<b>Color medio (RGB):</b> (" + QString::number(meanR) + ", " + QString::number(meanG) + ", " + QString::number(meanB) + ")<br/>";
    texto += "<div style=\"width:48px;height:24px;border:1px solid #000;background-color:rgb(" +
             QString::number(meanR) + "," + QString::number(meanG) + "," + QString::number(meanB) + ");\"></div>";

    QMessageBox info(w);
    info.setWindowTitle("Información de la imagen");
...
static int last_trazo_x = -1, last_trazo_y = -1;
...
//---------------------------------------------------------------------------
// Dibuja un trazo continuo entre (x1,y1) y (x2,y2) estampando puntos
// utilizando la misma lógica que cb_punto, pero realizando un único imshow
// al final para evitar discontinuidades cuando el ratón se mueve rápido.
void trazo_segmento(int factual, int x1, int y1, int x2, int y2, Scalar color)
{
    ...
    if (dist < 1.0) {
        // corto: pintar un único punto
        if (difum_pincel==0) {
            circle(im, Point(x1, y1), radio_pincel, color, -1, LINE_AA);
        } else {
            int t = radio_pincel + difum_pincel;
            int posx=t, posy = t;
            Rect roi(x1-t,y1-t,2*t+1,2*t+1);
            if (roi.x < 0) { roi.width += roi.x; posx+=roi.x; roi.x = 0; }
            if (roi.y < 0) { roi.height += roi.y; posy+=roi.y; roi.y = 0; }
            if (roi.x + roi.width > im.cols) roi.width = im.cols - roi.x;
            if (roi.y + roi.height > im.rows) roi.height = im.rows - roi.y;
            Mat trozo = im(roi);
            Mat res(trozo.size(), im.type(), color);
            Mat cop(trozo.size(), im.type(), CV_RGB(0,0,0));
            circle(cop, Point(posx, posy), radio_pincel, CV_RGB(255,255,255), -1, LINE_AA);
    ...
    // paso recomendado entre estampados: la mitad del radio para cubrir huecos
    double step = std::max(1.0, radio_pincel / 2.0);
    int steps = (int)std::ceil(dist / step);

    for (int i = 0; i <= steps; ++i) {
        double t = (steps==0) ? 0.0 : (double)i / (double)steps;
        int xi = cvRound(x1 + dx * t);
        int yi = cvRound(y1 + dy * t);

        if (difum_pincel==0) {
            circle(im, Point(xi, yi), radio_pincel, color, -1, LINE_AA);
        } else {
            int rt = radio_pincel + difum_pincel;
            int posx=rt, posy = rt;
            Rect roi(xi-rt, yi-rt, 2*rt+1, 2*rt+1);
            if (roi.x < 0) { roi.width += roi.x; posx+=roi.x; roi.x = 0; }
            if (roi.y < 0) { roi.height += roi.y; posy+=roi.y; roi.y = 0; }
            if (roi.x + roi.width > im.cols) roi.width = im.cols - roi.x;
            if (roi.y + roi.height > im.rows) roi.height = im.rows - roi.y;
...
void callback (int event, int x, int y, int flags, void *_nfoto)
{
    ...
    // 2.?: Herramienta TRAZOS (pincel estilo lápiz — líneas continuas entre posiciones)
    case HER_TRAZOS:
        if (event==EVENT_LBUTTONDOWN) {
            last_trazo_x = x;
            last_trazo_y = y;
            // draw initial point using trazo function to keep behavior consistent
            trazo_segmento(factual, x, y, x, y, color_pincel);
        } else if (event==EVENT_MOUSEMOVE && flags==EVENT_FLAG_LBUTTON) {
            if (last_trazo_x != -1) {
                trazo_segmento(factual, last_trazo_x, last_trazo_y, x, y, color_pincel);
            }
            last_trazo_x = x;
            last_trazo_y = y;
...
void ver_suavizado (int nfoto, int ntipo, int tamx, int tamy, bool guardar)
{
    ...
    // Esto lo hemos cambiado nosotros
    else if (ntipo == 3)
...
void ajuste_linea_hist(int nfoto, double pmin, double pmax, bool guardar)
{
...
//---------------------------------------------------------------------------
// Copiar ROI al portapapeles del sistema usando QClipboard
// Convierte la ROI (Mat BGR) a QImage y la pone en el portapapeles.
// En Windows se podrá pegar en Paint, Word, etc.
void copiar_roi_al_portapapeles(int nfoto)
{
    if (nfoto < 0 || nfoto >= MAX_VENTANAS || !foto[nfoto].usada) {
        QMessageBox::warning(nullptr, "Copiar al portapapeles", "No hay ninguna imagen activa.");
        return;
    }

    Rect roi = foto[nfoto].roi;
    if (roi.width <= 0 || roi.height <= 0) {
        QMessageBox::warning(nullptr, "Copiar al portapapeles", "La selección (ROI) está vacía.");
        return;
    }

    Mat roiMat = foto[nfoto].img(roi).clone();
    if (roiMat.empty()) {
        QMessageBox::warning(nullptr, "Copiar al portapapeles", "No se pudo obtener el ROI.");
        return;
    }

    // Convertir BGR (OpenCV) a RGB para QImage
    Mat tmp;
    if (roiMat.channels() == 3) {
        cvtColor(roiMat, tmp, COLOR_BGR2RGB);
        QImage qimg(tmp.data, tmp.cols, tmp.rows, static_cast<int>(tmp.step), QImage::Format_RGB888);
        QImage copy = qimg.copy(); // asegurar que los datos son propios
        QClipboard *cb = QApplication::clipboard();
        cb->setImage(copy);
    } else if (roiMat.channels() == 4) {
        cvtColor(roiMat, tmp, COLOR_BGRA2RGBA);
        QImage qimg(tmp.data, tmp.cols, tmp.rows, static_cast<int>(tmp.step), QImage::Format_RGBA8888);
        QImage copy = qimg.copy();
        QClipboard *cb = QApplication::clipboard();
        cb->setImage(copy);
    } else if (roiMat.channels() == 1) {
        QImage qimg(roiMat.data, roiMat.cols, roiMat.rows, static_cast<int>(roiMat.step), QImage::Format_Grayscale8);
        QImage copy = qimg.copy();
        QClipboard *cb = QApplication::clipboard();
        cb->setImage(copy);
    } else {
        QMessageBox::warning(nullptr, "Copiar al portapapeles", "Formato de imagen no soportado para copiar.");
        return;
    }
}
...
{
    QString nombres[3]= {":/imagenes/arena.jpg",
                          ":/imagenes/cielo.jpg",
                          ":/imagenes/madera.jpg"};
    ...
    imshow("Efecto de bajorrelieve", res);
