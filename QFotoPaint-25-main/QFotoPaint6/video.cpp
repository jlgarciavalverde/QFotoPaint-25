...
void star_wars (int nfoto, string nombre, int nframes, double fps, int codec, vector<string> texto, Scalar color, double escala, bool guardar)
{
    VideoWriter wri;
    if(guardar)
    {
    ...
    {
        putText(imTexto, texto[i], Point(0, h_vent+1.5*h_linea*(i+1)), FONT_HERSHEY_DUPLEX, escala, color, escala*1.5);
    }
...
void capturar_de_camara(int nres)
{
    ...
    if (tecla != 27) // La tecla no sea ESC, creamos una imagen con lo capturado de la cámara
    {
