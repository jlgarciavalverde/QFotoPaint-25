...
enum tipo_herramienta {HER_ARCOIRIS, HER_ELIPSE, HER_PUNTO, HER_TRAZOS, HER_LINEA, HER_RECTANGULO, HER_SELECCION};
...
// Copia el ROI de la foto indicada al portapapeles del sistema (QClipboard)
// En Windows se puede pegar en Paint u otras aplicaciones que acepten imágenes.
...
// Aplica una transformación de perspectiva desde `porig` hacia `pdest`.
// Si `guardar` es true, guarda el resultado en `ndest`.
void ver_perspectiva(int norig, int ndest, Point2f porig[], Point2f pdest[], bool guardar = false);
...
void ajuste_linea_hist(int nfoto, double pmin, double pmax, bool guardar = false);
// Operacion de ajuste linea del histograma
